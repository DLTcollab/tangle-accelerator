/*
 * Copyright (C) 2019-2010 BiiLabs Co., Ltd. and Contributors
 * All Rights Reserved.
...
static status_t insert_transaction(CassSession* session, const tryte_t* hash, const tryte_t* trytes) {
  ...
  if (cass_statement_bind_bytes_by_name(statement, "hash", (cass_byte_t*)hash, NUM_TRYTES_HASH) != CASS_OK) {
    ta_log_error("Fail to bind value to cass_statement\n");
    ret = SC_STORAGE_CASSANDRA_QUERY_FAIL;
    goto exit;
  }
  if (cass_statement_bind_bytes_by_name(statement, "bundle", (cass_byte_t*)(trytes + BUNDLE_OFFSET),
                                        NUM_TRYTES_BUNDLE) != CASS_OK) {
    ta_log_error("Fail to bind value to cass_statement\n");
    ret = SC_STORAGE_CASSANDRA_QUERY_FAIL;
    goto exit;
  }
  if (cass_statement_bind_bytes_by_name(statement, "address", (cass_byte_t*)(trytes + ADDRESS_OFFSET),
                                        NUM_TRYTES_ADDRESS) != CASS_OK) {
    ta_log_error("Fail to bind value to cass_statement\n");
    ret = SC_STORAGE_CASSANDRA_QUERY_FAIL;
    goto exit;
  }
  if (cass_statement_bind_bytes_by_name(statement, "tag", (cass_byte_t*)(trytes + OBSOLETE_TAG_OFFSET),
                                        NUM_TRYTES_OBSOLETE_TAG) != CASS_OK) {
    ta_log_error("Fail to bind value to cass_statement\n");
    ret = SC_STORAGE_CASSANDRA_QUERY_FAIL;
    goto exit;
  }

  if (cass_statement_bind_bytes_by_name(statement, "trytes", (cass_byte_t*)trytes, NUM_TRYTES_SERIALIZED_TRANSACTION) !=
      CASS_OK) {
    ta_log_error("Fail to bind value to cass_statement\n");
    ret = SC_STORAGE_CASSANDRA_QUERY_FAIL;
    goto exit;
...
static status_t insert_approvee(CassSession* session, const tryte_t* hash, const tryte_t* approvee) {
  ...
  if (cass_statement_bind_collection(statement, 0, collection) != CASS_OK) {
    ta_log_error("Fail to bind value to cass_statement\n");
    ret = SC_STORAGE_CASSANDRA_QUERY_FAIL;
    goto exit;
  }
  if (cass_statement_bind_bytes(statement, 1, (cass_byte_t*)hash, NUM_TRYTES_HASH) != CASS_OK) {
    ta_log_error("Fail to bind value to cass_statement\n");
    ret = SC_STORAGE_CASSANDRA_QUERY_FAIL;
    goto exit;
  }
...
static status_t get_inclusion_status_from_transaction_table(CassSession* session, get_inclusion_states_res_t* res,
...
static status_t get_hash_from_transaction_table(CassSession* session, hash243_queue_t* res_queue,
  ...
  statement = cass_prepared_bind(prepared);
  if (cass_statement_bind_bytes_by_name(statement, where_column, (cass_byte_t*)where_value, length) != CASS_OK) {
    ta_log_error("Fail to bind value to cass_statement\n");
    ret = SC_STORAGE_CASSANDRA_QUERY_FAIL;
    goto exit;
  }
...
status_t db_client_get_inclusion_states(const db_client_service_t* service, get_inclusion_states_req_t const* const req,
  ...
  CDL_FOREACH(req->transactions, itr243) {
    ...
    if (ret != SC_OK) {
      ta_log_error("Fail to get inclusion status\n");
      return ret;
    }
