...
static void* listener_handler(void* in) {
  ...
  char tx_hash[NUM_FLEX_TRITS_HASH + 1];
  ta_log_info("Start listening from ZMQ server : %s\n", zmq_server);
  /* setting ZeroMQ */
  void* context = zmq_ctx_new();
  void* subscriber = zmq_socket(context, ZMQ_SUB);
  if (subscriber == NULL) {
    ta_log_error("Failed to establish subscriber\n");
    goto exit;
  }
  if (zmq_connect(subscriber, zmq_server) != 0) {
    ta_log_error("Failed to connect to zmq server : %s\n", zmq_server);
    goto exit;
  }
  ...
  if (zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE, "sn", strlen("sn")) != 0) {
    ta_log_error("Failed to set setsockopt : sn\n");
    goto exit;
  }
#endif
  ...
  while (1) {
...
#ifdef IRI_SUPPORT_SN_TRYTES
    flex_trit_t tx_trytes[NUM_FLEX_TRITS_SERIALIZED_TRANSACTION + 1];
    sscanf(zmq_receive_string, "sn_trytes %s %s", ta_trytes, tx_hash);
    hash8019_queue_push(&tx_res, ta_trytes);
    hash243_queue_push(&ta_req->hashes, (flex_trit_t const* const)tx_hash);
#else

    int milestone_idx;
    sscanf(zmq_receive_string, "sn %d %s", &milestone_idx, tx_hash);
    ta_log_debug("Get hash %s\n", tx_hash);
    if (hash243_queue_push(&tx_req->hashes, (flex_trit_t const* const)tx_hash) != RC_OK) {
      ta_log_error("%s\n", "SC_STORAGE_OOM");
      goto loop_end;
    }
    if (iota_client_get_trytes(data->service, tx_req, tx_res) != RC_OK) {
      ta_log_error("Failed to get trytes from IRI\n");
    }
    ...
    while (hash8019_queue_empty(tx_res->trytes) == false) {
      while (db_permanode_thpool_add((tryte_t*)tx_req->hashes, (tryte_t*)tx_res->trytes->hash, data->pool) != SC_OK) {
        /** The request queue is full or unavailable */
...
exit:
  ta_log_info("Exit db listener handler\n");
  free(zmq_server);
...
int main(int argc, char* argv[]) {
  ...
  ta_log_info("Connect db_host = %s, iri_host = %s\n", db_host, iri_host);
  ...
  if (db_permanode_thpool_init(&pool) != SC_OK) {
    ta_log_error("Failed to init permanode threadpool\n");
    return EXIT_FAILURE;
  }
  /* create the request-handling threads */
  for (int i = 0; i < thread_num; i++) {
    if (db_permanode_thpool_init_worker(worker_data + i, &pool, db_host) != SC_OK) {
      ta_log_error("Failed to init permanode thpool worker\n");
      return EXIT_FAILURE;
    }
