...
void test_pow_bundle(void) {
  ...
  BUNDLE_FOREACH(bundle, tx_iter) {
    TEST_ASSERT_FALSE(memcmp(transaction_trunk(tx_iter), hash_trits_1,
                             sizeof(flex_trit_t) * FLEX_TRIT_SIZE_243));
    TEST_ASSERT_FALSE(memcmp(transaction_branch(tx_iter), hash_trits_2,
                             sizeof(flex_trit_t) * FLEX_TRIT_SIZE_243));
  }
