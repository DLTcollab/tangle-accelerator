...
void test_fetch_txn_with_uuid_res_sent_serialize(void) {
  const char* json =
      "{\"hash\":"
      "\"QNPHSOPESISPNHQSESOP9IHMAPZBNJHTXDFDGFIVSHFF9OEJIZHHZCGABEJ9MAX9QMIKZGAYTXN999999\",\"signature_and_message_"
      "fragment\":"
      ...
      "timestamp_upper_bound\":0,\"nonce\":\"NC99999999XRVYE999999999999\"}";
  char* json_result = NULL;
  ...
  flex_trits_from_trytes(txn_trits, NUM_TRITS_SERIALIZED_TRANSACTION, (const tryte_t*)TRYTES_2673_1,
                         NUM_TRYTES_SERIALIZED_TRANSACTION, NUM_TRYTES_SERIALIZED_TRANSACTION);
  res->txn = transaction_deserialize(txn_trits, true);
